...
internal static class PotionUsePolicy
{
    ...
    /// <summary>
    /// 低保留价值药水（如 FruitJuice 永久 +最大生命）：不是"始终战斗内使用"——
    /// 而是跑局级"占位药水"：栏位满且要拿新药时在战斗外喝掉腾栏位（由 RunAuto 槽位管理负责）。
    /// 战斗内保留普通门槛：只有能救场（无药路线会死）或省血达标时才用，避免无谓消耗。
    /// </summary>
    public static bool IsLowValueFiller(PotionModel potion)
