...
| 评分 AI 五角色验证（`run-picker-checks-all-chars.ps1`） | 通过（headless，5/5 角色） | 对 IRONCLAD/SILENT/DEFECT/NECROBINDER/REGENT 各开一局跑同一组评分断言 + `PICKER_SCORES` 实际评分日志。验证各角色初始牌组上下文下的评分正确性；发现 DEFECT 初始攻击少（4/10），注入牌组后攻击比例 <0.3 触发 +9 加成（头槌重复 10 分、第 2 幕燃烧 38 分、打击第 2 幕 0 分），其余角色不触发（1/29/-9）——按角色参数化期望后 5 角色全过。 | 2026-09-02 |
