...
| 跨职业渠道选取率强化 + 可见跑局脚手架（`CHANNEL-PICK-RATE`，提交 2e631b9 起） | 部分通过 | ①评分：万花筒 Neow 4→7、海玻璃 Orobas 池 4→9、棱彩 6→8、TouchOfOrobas 6→7；先古排序断言 KALEIDOSCOPE>GOLDEN_PEARL/ARCANE_SCROLL、SEA_GLASS>PRISMATIC_GEM/TOUCH_OF_OROBAS 全 5 角色 `ALL_CHARS_OK`。②脚手架：请求/设置 `RunAutoForceNeowRelicId`（Neow 强制选该先古）与 `RunAutoForceActRelicId/Act`（指定幕首个事件房入口强制获得；海玻璃需 fire-and-forget——其 `AfterObtained` 用 BlockingPlayerChoiceContext 等网格选择，await 会与覆盖层驱动互锁）。headless 冒烟（IRONCLAD/seed1/幕0 事件房强制 SEA_GLASS）：`脚手架获得` → 网格弹出 → **择优 5 张 Silent 牌**（ASSASSINATE 35.6/THE_HUNT 23.8/SHADOWMELD 22.3/ABRASIVE 20.6/KNIFE_TRAP 20.2，接收职业校准分）入组，运行正常收尾 exit 0。可见跑局验证（开局万花筒两轮奖励、幕2 海玻璃、色彩哲学家）待实机。 | 2026-09-04 |
