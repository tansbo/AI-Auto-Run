...
| 果汁药水政策（用户设计修正，`POTION-JUICE-ALWAYS-USE` 已作废） | 代码回滚（Release 构建通过）；设计实现待办 | 曾实现"FruitJuice（永久 +5 最大生命）战略成本 0 → Smart 政策战斗内始终建议使用"并跑通 headless fixture（毛绒伏地虫 40HP + 注入 FRUIT_JUICE，断言终局最大生命 `85`）。**用户纠正设计**：果汁不是保留成本 0 / 战斗内永远用——而是**占位药水**：栏位满且要拿新药时在**战斗外**喝掉腾栏（既得 +5 又不丢新药），战斗内维持普通救场门槛。代码已回滚（普通战略成本 + `IsLowValueFiller` 标记），原 fixture 断言不再适用（回滚后攒住果汁终局为 `80`）；战斗外"喝占位药腾栏"由 RunAuto 槽位管理实现（见下行 `POTION-SLOT-MGMT`）。 | 2026-09-02 |
| 跑局药水/篝火经济决策层（`POTION-SLOT-MGMT` / `RUN-ECONOMICS`） | 代码完成（Release 构建 0 错误）；行为待实机回归 | 用户规则落码：① 战斗外只喝 {果汁, 鲜血药水}，其余腾栏只能丢弃（decomp 核对 AnyTime + 用药通道 `EnqueueManualUse`/`PotionCmd.Discard`）；② 保留分随路线危险度与幕末 Ancient 补血（A2+ 补缺失 80%）调整，满栏药水奖励决策腾栏/跳过，全部进日志；③ 篝火幕末 Boss 前按失败风险回血而非敲牌（含 A10 双 Boss/药水兜底信号）。启发式阈值待多种子实机回归校准。 | 2026-09-02 |
...
| 评分 AI 五角色验证（`run-picker-checks-all-chars.ps1`） | 通过（headless，5/5 角色） | 对 IRONCLAD/SILENT/DEFECT/NECROBINDER/REGENT 各开一局跑同一组评分断言 + `PICKER_SCORES` 实际评分日志。验证各角色初始牌组上下文下的评分正确性；发现 DEFECT 初始攻击少（4/10），注入牌组后攻击比例 <0.3 触发 +9 加成（头槌重复 10 分、第 2 幕燃烧 38 分、打击第 2 幕 0 分），其余角色不触发（1/29/-9）——按角色参数化期望后 5 角色全过。**2026-09-02 更新**：接入 Spire Codex A10 真实胜率差数据驱动加成后按公式校准期望（战吼 31→32.845、DEFEND_DEFECT −13.995、STRIKE_SILENT +2.017 等，精确到小数），`ALL_CHARS_OK` 5/5 通过；先古诅咒遗物（CursedPearl 净正）改为按净价值可选中。 | 2026-09-02 |
| 数据驱动评分可见整局回归（`DD-VISIBLE`，COMBATSOLVER/IRONCLAD） | 通过（可见整局） | 483 卡胜率差加成 + 事件/先古估值层全开，48 层 Act3 正常阵亡（1306s），`FULL_RUN PASSED`、无死路；层数与数据驱动前同种子持平（48），确认卡牌数据驱动评分不劣化整局。 | 2026-09-02 |
| 遗物遥测 + 语料基座（`RELIC-TELE`，COMBATSOLVER/IRONCLAD） | 通过（可见整局） | 遥测新增本局获得遗物清单（奖励/宝箱/事件/先古四路，修复 `RecordRelicPick` 死代码），实机局 48 层 Act3 记录 11 件遗物（含 Neow 药瓶皮套、Orobas 玻璃眼珠事件获取）；`tools/aggregate_relic_corpus.py` 聚合 `relicIds×胜负` 语料 CSV——遗物侧数据校准（公开源不可得后）走该内部语料。 | 2026-09-02 |
...
| 篝火敲牌真实增量（`SMITH-UPGRADE-DELTA`） | 代码完成（Release 构建 0 警告 0 错误）；实机行为待整局复测 | `SmithUpgradeDriver` 升级选牌不再用静态稀有度启发式：对每张可升级牌克隆出"敲后"模型（官方预览同款路径 `ICardScope.CloneCard` + `UpgradeInternal` + `FinalizeUpgradeInternal`，同 `FromSerializable` 语义），评分 = 敲后相对敲前的 **DynamicVar 数值增量之和（max(0,Δ) 防自伤符号误判）** + 费用降低 ×15 + 稀有度同分决胜；克隆不可用时退化为旧启发式兜底。已升级牌仍不选。改动只影响篝火 Smith 升级目标选择，不影响战斗搜索。 | 2026-09-04 |
| 选牌体系契合 + 卡组画像扩展（`PICKER-ENGINE-SYNERGY`） | 通过（headless，5/5 角色 `ALL_CHARS_OK`） | `CardPickerAI` 评分新增**卡组体系契合**：机械轴（消耗/小刀 Shiv/随从 Minion/奥斯蒂攻击 OstyAttack/毒/Doom/力量/过牌-能量/零费，全部取自卡牌自身关键词·CardTag·DynamicVar·费用，非手写卡表）在牌组里已成型（≥1 张）时，同轴候选 +2×min(3, 同轴数)；未启动的轴不加分。`DeckContext` 增加 9 个轴计数。职业特征经由数据驱动加成（按角色池中位，Spire Codex A10）与各职业独有轴（Shiv=静默、Doom/Minion/Osty=亡灵等）自然表达。fixture 复跑发现并校准一处真实行为差：SILENT（初始 12 张，含幸存者）与 REGENT（初始 10 张，含抽牌/能量轴起始牌）初始牌组带抽牌轴 → 战吼（CardsVar）单卡期望 32.845→34.845 按角色参数化；其余检查期望不变，`ALL_CHARS_OK` 5/5 通过。 | 2026-09-04 |
| 跨职业接收职业校准 + 海玻璃任意选择优（`CROSS-CLASS-VALUE`，提交 59bdadf） | 通过（headless，5/5 角色 `ALL_CHARS_OK`） | 渠道取证（decomp）：万花筒/色彩哲学家/棱彩宝石→他职业牌走标准卡奖励流（已覆盖），海玻璃→`NSimpleCardSelectScreen` min0 网格（旧驱动"确认可用即确认"=一张不拿 → 修复为按 `CardPickerAI` 择优 ≤5 张再确认）。`CardWinStats` 快照改数据表（绝对胜率/归属池/各池中位），`BonusFor(entry, receivingRole)`：同池对照自身池中位（IRONCLAD 全部期望不变即旁证）、跨池/无色对照接收职业中位；`DeckContext.ReceivingRole` 接入。他职业候选（战吼/燃烧/头槌/壁垒）fixture 期望按接收职业逐角色重校，`ALL_CHARS_OK` 5/5。海玻璃等渠道整局行为待实机回归。 | 2026-09-04 |
| 语义配合表（`COMBO-SYNERGY`，`CardComboProfiles.cs`） | 通过（headless，5/5 角色 `ALL_CHARS_OK`） | 子代理逐卡精读核对用户点名配对：UNRELENTING×SEVEN_STARS 不成立（`FreeAttackPower` 只免能量不免星能，7★ 瓶颈仍在）、SEVEN_STARS×GRAND_FINALE 无互触发；唯一真实强配合 STAMPEDE×GRAND_FINALE（`CardCmd.AutoPlay` 不检查 `IsPlayable`，惊逃自动打出绕过华丽收场空抽牌堆门槛免费 AOE，跨职业渠道混池可达）。`CardComboProfiles` 仅保留 STAMPEDE⇄GRAND_FINALE 双方向 +6（`DeckContext.ContainsAny` 查牌组含伙伴）；全角色矩阵新增配合断言 GRAND_FINALE+[STAMPEDE] 期望逐角色重校，`ALL_CHARS_OK` 5/5。 | 2026-09-04 |
| 跨职业渠道选取率强化 + 可见跑局脚手架（`CHANNEL-PICK-RATE`，提交 2e631b9 起） | 通过 | ①评分：万花筒 Neow 4→7、海玻璃 Orobas 池 4→9、棱彩 6→8、TouchOfOrobas 6→7；先古排序断言 KALEIDOSCOPE>GOLDEN_PEARL/ARCANE_SCROLL、SEA_GLASS>PRISMATIC_GEM/TOUCH_OF_OROBAS 全 5 角色 `ALL_CHARS_OK`。②脚手架：请求/设置 `RunAutoForceNeowRelicId`（Neow 强制选该先古）与 `RunAutoForceActRelicId/Act`（指定幕首个事件房入口强制获得；海玻璃 `AfterObtained` 用 BlockingPlayerChoiceContext 阻塞等网格 → 必须 fire-and-forget 由覆盖层驱动排空，await 会互锁，已实机复现并修复；`CharacterId` 预置非当前职业）。③**可见跑局（IRONCLAD/COMBATSOLVER，DemoShot 帧）**：万花筒局——跨职业卡奖励正常弹出择优（含 Necrobinder GRAVE_WARDEN 等他职业牌）目标达成；海玻璃局——幕 2 首事件（OROBAS）强制获得 → 15 张 Silent 牌网格 → **择优 5 张入组**（BLADE_OF_INK 28.2/EXPOSE 25/MALAISE 23/THE_HUNT 21.8 等，按接收职业校准分），随后 Orobas 事件正常继续，无卡死。色彩哲学家四色档仍并列（无更准取舍依据不硬排）；其奖励流驱动覆盖早前已核实。 | 2026-09-04 |
| 卡牌能力四维画像 + 分幕需求补位 + 联动互乘（`PICKER-FOUR-DIM-GAP`） | 通过（headless，5/5 角色 `ALL_CHARS_OK`） | `DeckGapBonus` 升级为**四维补位**：新增 `CardAbilityProfile.cs` 单卡四维画像（攻击=Damage×Repeat（AOE 对单等效 ×1.25/1.1、X 费按名义 X=2）；防御=Block 变量或 GainsBlock 保守 4；回费=Energy 变量 + Canonical≤0 免费牌 +1（星能不折算）；过牌=Cards 变量 + Retain 0.5），`DeckContext.AbilityTotals` 惰性合计缓存。`DeckGapFill` = 逐维 max(0, 分幕需求×clamp(deck/12,0.8,1.5) − 牌组合计) 的覆盖比例 × 权重（4/4/3/3.5，合计封顶 10，≥3 同名不补位）；需求幕基值 act0 攻/防双高、act1 攻+过牌、act≥2 防御+高攻击。新增 `SynergyAmplify`：牌组有 VulnerablePower 变量时 DOMINATE/COLOSSUS +5（decomp Dominate.cs L40-42、ColossusPower.cs L27-46）；≥3 费攻击在牌组有 Energy 变量卡或 0 费卡 ≥3 时 +3。**v2 精化**：新增 `RoleStatusAccess.cs` 职业×状态产出指数（运行时真实枚举 ModelDb.AllCharacters→CardPool.AllCards，按 DynamicVar 键去重计数并惰性缓存；实测 StrengthPower IC=7/其余 0-1、VulnerablePower IC=6/REGENT=5/NECRO=2/SILENT=1/DEFECT=1）——多段攻击画像 ×clamp(StrengthPower/中位,0.8,1.25)、DOMINATE/COLOSSUS 加成 ×clamp(VulnerablePower/中位,0.8,1.3)；墨灵每段减免 BossAdjust 不动。**v3 精确机制**（第二份取证报告融入）：乘算只吃 powered（毒/球/不具名不吃）、虚弱 ×0.75/易伤 ×1.5/易碎格挡 ×0.75、Debilitate 加深易伤 1.5→2.0 压虚弱 0.75→0.5、负/临时力量=每段 -X；多段画像改按 **StrengthPower+VulnerablePower 合计产出**相对中位缩放（实测合计 IC=13/REGENT=6/NECRO=3/DEFECT=2/SILENT=1，中位 3 → IC/REGENT ×1.25、NECRO ×1.0、SILENT/DEFECT ×0.8）；主宰/巨像注释按 Dominate.cs/Colossus.cs/ColossusPower.cs 行号精化（施放后总易伤层数 V 换常驻力量、带易伤 powered 攻击者对 Owner ×0.5），补五职业核心折算口径。fixture 全角色重校（战吼 −5、华丽收场/第2幕燃烧 +2~7、头槌 12 防御局 +1~2，≥3 同名不变；新增 DOMINATE/COLOSSUS 注入 BASH 对照与 Conf12 多段缩放行，实测注入增幅 IC/REGENT +6.5、NECRO +5、SILENT/DEFECT +4；v3 多段缩放重校 DEFECT/REGENT 行）；Release 0 警告 0 错误；`REFACTOR_BOUNDARIES_OK`。**v4 浓度-凸收益/每回合引擎/可达性**：新增 `ConcentrationProfiles.cs`——C 可达性系数（0/1 费占比 + Cards 抽牌密度 → clamp(0.6+0.4×廉价+2.5×密度,0.7,1.3)，DeckAbilityTotals 增 CheapCards）；A DEATHS_DOOR 浓度档（本回合施过 Doom→格挡×3=18/21，DeathsDoor.cs L21-23/L31-37；DoomPower 轴 0→0、≥1 +3、≥3 +6，×可达性）；B COUNTDOWN 每回合引擎（CountdownPower.cs L26-36：每玩家回合随机敌 +Doom 6/9；基准 × 幕节奏 × 可达性 − 鬼抽罚）；"精神过载"NEUROSURGE 核对为对 Owner 自施 Doom（NeurosurgePower.cs L20-27）不入表。fixture 新增 DEATHS_DOOR 三档（无注入/OBLIVION×1/×3）与 COUNTDOWN 行，全角色重校（IC Dd0 9→Dd1 11.871→Dd3 16.76、Cd 16.52 等），`ALL_CHARS_OK` 5/5。**v5 浓度动态择优落库 + 放大器口径修正**：新增 `EngineConcentration.cs`（engine-catalog 普查 210→单人 200 落库：显式规则把 190 条映射到 11 个可测"浓度维度"，unclassified 6 不入表；v1-v4 专表条目剔除防双计）+ `DeckContext.ConcentrationOf`（Vulnerable/Doom/Poison 变量总量、Exhaust/Ethereal 关键词、抽弃、召唤、廉价/攻/技计数）+ 浓度加分（cap≤5，凸型浓度² 放坡）+ PickBest 同机制上行平局取舍；多段画像按"每段放大器源指数"修正（IC=力量+易伤 13/静默=易伤+小刀 2（不再 0.8）/亡灵=易伤+Doom+Osty 29 不含力量/故障·君王保守近似；中位 6 → IC·亡灵 ×1.25 其余 ×1.0）；Friendship/SharedFate 自减力量负分按"玩家出手 FromCard 攻击依赖度"缩放（DeckContext.PlayerAttackReliance 惰性启发式：Osty 不计入；主走 Osty/Doom 体系豁免），Osty 攻击不吃玩家力量等不存在的边不加；fixture 新增 BULLY/MOLTENFIST/TIMESUP/FRIENDSHIP(+INFLAME×2) 行全角色重校，`ALL_CHARS_OK` 5/5。 | 2026-09-05 |
...
3. 开始单人铁甲战士跑局（任意种子），依次核对：
   ...
   - 战后自动选牌/跳过、逐个领取奖励、Boss 遗物自动选择；
   - 战斗内战斗求解器全自动接管并打完；
   - 商店自动买卡/买遗物/买药水/移除、篝火低血量回血否则升级、事件自动选非致死选项、宝箱自动拾取。
4. 至少跑到 Act1 Boss 并打完（或阵亡收尾），全程无卡死、无无限等待。
5. 出现异常（错误路线、卡死、计划外重算）时：设置 → 反馈 → 上传问题包，或本地导出后提交。
